...
- Map zoom is the map's limit, tile depth is the source's: the map widgets go to `MAP_MAX_ZOOM` (24, `packages/utils/src/basemap-providers.ts`), which is deeper than anything any basemap publishes — deliberately, because these maps are read at robot scale and MapLibre re-rasterises the vector geometry ORMI draws on top at the real zoom while the basemap underneath is merely stretched. Three limits are involved and only one of them is a cap. **(1)** The **map's** `maxZoom` prop, on both `<MapLibreMap>` elements. **(2)** The raster **source's** `maxzoom`, which is how deep the vendor actually publishes (`RasterBasemapProvider.maxSourceZoom`, required on every catalogue entry, resolved through `basemapMaxSourceZoom`): declared, MapLibre stops requesting tiles the vendor never generated and overzooms the deepest real level; undeclared, the style spec defaults it to 22 and OpenStreetMap (19), OpenTopoMap (17) and Esri (19) 404 their way to a **blank** basemap on the way in. An unknown url — an operator's own tile server — declares nothing, because capping it would be a guess. **(3)** A **layer** `maxzoom`, which is not a cap but a _hide_: the layer stops drawing at and above it. `maxzoom: 22` on `simple-tiles` is what actually made the basemap disappear past z22, and it must never come back; asserted on both builders (`use-map-style.test.ts`, `hooks/__tests__/useMapStyle.test.ts`) because the two are separate code with one contract. Past the basemap's own depth its detail no longer indicates distance, so both maps render **`MapChrome`** unconditionally (`@workspace/utils/map-chrome`) — a north indicator and a metric scale bar in ONE positioned cluster. Both are always on screen rather than appearing on a state change: an indicator an operator has not learnt the position of is one they cannot find at the moment they need it. The cluster is one placement decision, which is the point — a scale bar placed independently at `bottom-2 left-2` sat straight on top of the standard map's `UnconfiguredEntriesNotice`, so a third indicator later inherits a corner instead of guessing one and a magic offset. Neither is MapLibre's built-in control: `maplibre-gl.css` gives `.maplibregl-ctrl-group` a hardcoded white background, so `ScaleControl`/`NavigationControl` render as a white box on the dark theme, and the stock scale bar rounds through `10 ** (String(Math.floor(d)).length - 1)`, which is `1` for every distance under 10 m and so cannot express a bar below one metre — it reports "1 m" for a 0.8 m bar and draws it 25% too long, in exactly the range this zoom ceiling puts operators in. **Rotation is enabled on both maps** (MapLibre's default `dragRotate`), so the compass is not decoration: it takes the accent colour and states the heading in figures when off north — an arrow gives a direction, only a number gives a bearing — it tilts the needle with pitch, and clicking it calls `resetNorth` and never `resetNorthPitch`, because the standard map **opens** at `pitch: 45` and flattening it would undo the widget's own default view in answer to a request about rotation. Both maps take ONE `move` subscription for the whole cluster, through `useSyncExternalStore` over an identity-stable snapshot quantised before it becomes state (3 s.f. on the resolution, 1 d.p. on the angles), or a pan re-renders it once per animation frame. The arithmetic is pure and unit-tested in `map-scale.ts` and `map-orientation.ts` — rounding before folding, so a bearing of 359.6° reads `000°` and never `360°`; the component is kept OUT of the utils barrel and given its own subpath because it imports `react-map-gl` at runtime, per the worker-import rule. Standing limit, deliberately not papered over: on a pitched map a single scale bar is exact only along the centre line, which is where it samples.
