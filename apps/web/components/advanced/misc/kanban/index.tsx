...
export function KanbanView({
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    setWorkspaces((prev) => {
      ...
      const newCategoryId =
        overContainerStr === "category-uncategorized"
          ? null
    ...
    if (activeContainer && overContainer) {
      ...
      if (activeContainer === overContainer) {
        ...
        const insertIndex = Math.max(
          0,
          Math.min(newIndex, newOverItems.length),
        );
  ...
  return (
    ...
    >
      <div className="flex flex-wrap gap-4 pb-4">
          ...
          {categories.map((category) => {
            ...
            return (
              <KanbanColumn
                ...
                onEdit={(cat) => {
                  setEditingCategory(
                    categories.find((c) => c.id === cat.id) || null,
                  );
        ...
        {columns["category-uncategorized"] &&
          columns["category-uncategorized"].length > 0 && (
            <KanbanColumn
        ...
        <DragOverlay dropAnimation={dropAnimation}>
          {activeColumn ? (
            ...
            <div className="w-[300px]">
              <WorkspaceItem
                workspace={
                  workspaces.find((w) => `workspace-${w.id}` === activeId)!
                }
              />
            </div>
          ) : null}
