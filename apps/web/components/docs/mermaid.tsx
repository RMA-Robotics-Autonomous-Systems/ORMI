...
export function Mermaid({ chart, className, size = 'large' }: MermaidProps) {
    ...
    if (error) {
        return (
            <div className="my-6 rounded-lg border border-red-500/50 bg-red-50 p-4 dark:bg-red-950/20">
                ...
                <pre className="mt-2 text-xs text-red-600 dark:text-red-400">{error}</pre>
            </div>
        )
    }
    ...
    return (
        <div
            ref={containerRef}
            className={`my-6 flex justify-center overflow-x-auto ${size === 'large' ? 'scale-110' : size === 'small' ? 'scale-90' : ''
                } ${className || ''}`}
