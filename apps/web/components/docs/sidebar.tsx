...
function NavItem({ item, pathname, level = 0 }: { item: NavigationItem; pathname: string; level?: number }) {
        ...
        return (
            <SidebarMenuItem>
                <SidebarMenuButton asChild isActive={isActive}>
                    <Link href={item.href}>
                        {isHome ? <Home className="mr-2" /> : <FileText className="mr-2" />}
...
function SubNavItem({ item, pathname }: { item: NavigationItem; pathname: string }) {
    const isActive = pathname === item.href
    const hasChildren = item.children && item.children.length > 0
    const [isOpen, setIsOpen] = useState(isActive || pathname.startsWith(item.href))
            ...
            <SidebarMenuSubItem>
                ...
                <CollapsibleContent>
                    <div className="ml-4 border-l pl-2 space-y-1">
                        {item.children?.map((child) => (
                            <SidebarMenuSubButton key={child.href} asChild isActive={pathname === child.href}>
