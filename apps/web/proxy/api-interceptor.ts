...
export class ApiInterceptor {
  ...
  private async cacheResponse(
    ...
    try {
      if (pathname === API_ENDPOINTS.WORKSPACES && request.method === "GET") {
        // Cache all workspaces
        const workspaces = Array.isArray(responseData) ? responseData : [];
        ...
        await this.db.syncTemplates(
          templates.map((template) => ({
            id: parseInt(template.id || template.id?.toString() || "0"),
            name: template.name,
            content: template.widget || template.datasource || template.content,
            type: template.type,
  ...
  private async handleOfflineRequest(
    ...
    try {
      ...
      if (pathname === API_ENDPOINTS.WORKSPACES) {
        return this.handleWorkspacesRequest(method, request, auth.userId);
        ...
        return this.handleTemplatesRequest(method, request, auth.userId);
  ...
  private async handleWorkspaceByIdRequest(
    ...
    switch (method) {
      case "GET": {
        const workspace = await this.db.getWorkspace(workspaceId, userId);
        if (!workspace) {
          return new Response(
  ...
  private async handleTemplateByIdRequest(
    ...
    switch (method) {
      case "PUT": {
        const updateData = await request.json();
        const existingTemplate = await this.db.getTemplate(templateId, userId);
  ...
  private async handleAuthRequest(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (API_ENDPOINTS.AUTH_SIGNOUT.test(pathname) && response.ok) {
        console.log("🔓 Sign-out detected, clearing cached session data");
  ...
  private async cacheSessionData(response: Response): Promise<void> {
    try {
      ...
      if (sessionData && sessionData.user) {
        console.log("💾 Caching session data for user:", sessionData.user.id);
