...
export class ConflictResolver {
	...
	async resolveWorkspaceConflict(
		...
		console.log(
			`🔄 Resolving workspace conflict for ID: ${localWorkspace.id}`,
		);

		try {
			// Get the current server version
			const response = await fetch(
				`/api/workspaces/${localWorkspace.id}`,
			);
			...
			if (localTimestamp > serverTimestamp) {
				...
				console.log(
					"📥 Server workspace is newer, updating local copy",
				);
	...
	async resolveTemplateConflict(localTemplate: TemplateData): Promise<void> {
		console.log(
			`🔄 Resolving template conflict for ID: ${localTemplate.id}`,
		);
	 ...
	 */
	private async pushWorkspaceToServer(
		workspace: WorkspaceData,
	): Promise<void> {
	...
	private convertApiTemplateToLocal(
		...
		return {
			...
			content:
				(apiTemplate as any).widget ||
				(apiTemplate as any).datasource ||
				{},
	...
	async batchResolveWorkspaceConflicts(
		...
		console.log(
			`🔄 Batch resolving ${workspaces.length} workspace conflicts`,
		);

		const promises = workspaces.map((workspace) =>
			this.resolveWorkspaceConflict(workspace).catch((error) => {
				console.error(
					`Failed to resolve workspace ${workspace.id}:`,
					error,
				);
	...
	async batchResolveTemplateConflicts(
		...
		console.log(
			`🔄 Batch resolving ${templates.length} template conflicts`,
		);

		const promises = templates.map((template) =>
			this.resolveTemplateConflict(template).catch((error) => {
				console.error(
					`Failed to resolve template ${template.id}:`,
					error,
				);
