...
export class OfflineDatabase {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains("workspaces")) {
          ...
          workspaceStore.createIndex("_lastModified", "_lastModified", {
            unique: false,
          });
        }
        ...
        if (!db.objectStoreNames.contains("templates")) {
          ...
          templateStore.createIndex("public", "public", { unique: false });
          templateStore.createIndex("_lastModified", "_lastModified", {
            unique: false,
          });
        }
  ...
  async getWorkspaces(userId: string): Promise<WorkspaceData[]> {
    ...
    try {
      ...
      console.log("💾 Found", workspaces.length, "workspaces in offline DB");
      return workspaces;
    } catch (error) {
      console.error("❌ Error getting workspaces from offline DB:", error);
      return [];
    }
  }
  ...
  async getTemplate(id: number, userId: string): Promise<TemplateData | null> {
    const store = await this.getStore("templates");
  ...
  async clearAllData(): Promise<void> {
    ...
    const stores = ["workspaces", "templates", "users", "auth", "syncQueue"];
