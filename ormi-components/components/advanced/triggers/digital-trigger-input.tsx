...
export function getGamepadButtonName(index: number | undefined, gamepadId?: string): string {
    if (index === undefined) {
        return '?';
    }
...
export const DigitalInputComponent = (props: DigitalInputComponentProps) => {
    ...
    const [data, setData] = useState<DigitalInput | null>(props.data);
    ...
    useEffect(() => {
        const keyPressEvent = (event: KeyboardEvent) => {
            if (data && data.type === 'keyboard' && event.key.toLowerCase() === data.key?.toLowerCase()) {
                setIsKeyDown(true);
            }
                    ...
                    gamepad.buttons.forEach((button, index) => {
                        if (button.pressed) {
                            ...
                            if (isSelecting) {
                                setData({
                                    type: 'gamepad',
                                    gamepadButton: getGamepadButtonName(index, gamepad.id),
    ...
    return (
        <div style={{ width: '10rem' }}>
            <span data-active={isKeyDown} className={styles.key} onClick={handleSelecting}>
                {isSelecting ? (
                    'press'
                ) : data ? (
                    <div
                    ...
                    >
                        {data.type === 'keyboard' ? (
                            ...
                            <>
                                <GamepadIcon />
                                {getGamepadButtonName(data.gamepadButtonIndex, data.gamepadId)} {/* Use the helper function */}
                            </>
                        )}
                    </div>
                ) : (
