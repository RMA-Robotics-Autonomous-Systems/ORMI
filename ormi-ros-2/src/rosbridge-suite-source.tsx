...
const RosBridgeSuiteSourceProvider = (children: ReactNode, props: RosBridgeSuiteDataSourceSettings) => {
    ...
    const ros_publishers = useRef(new Map<string, RosTopicAndCounter>()).current; // Keep using useRef for consistency
    const advertisingPromisesRef = useRef<Map<string, Promise<boolean>>>(new Map()); // Topic name -> Promise<success> for advertise
    const unadvertisingPromisesRef = useRef<Map<string, Promise<void>>>(new Map()); // Topic name -> Promise<void> for unadvertise
    ...
    useEffect(() => {
            ...
            pluginsManager.addFilter(advertise_hook, {
                ...
                filter: async (topic: SelectedTopic): Promise<boolean> => {
                    ...
                    if (advertisingPromisesRef.current.has(topicName)) {
                        console.log(`ROS2 Advertise for ${topicName}: Operation already in progress, awaiting...`);
                    ...
                    const advertisePromise = (async (): Promise<boolean> => {
                        try {
                            console.log(`ROS2 Advertise for ${topicName}: Starting operation...`);
                            ...
                            if (ros_publishers.has(topicName)) {
                                const topic_and_counter = ros_publishers.get(topicName)!;
                                console.log(`ROS2 Advertise for ${topicName}: Publisher already exists, incrementing count.`);
                                topic_and_counter.counter++;
                                console.log(`ROS2 Advertise for ${topicName}: Count incremented to ${topic_and_counter.counter}.`);
                                return true; // Indicate success
                            }

                            // No existing publisher, proceed with advertising
                            console.log(`ROS2 Advertise for ${topicName}: No existing publisher, creating ROSLIB.Topic...`);
                            ...
                            console.log(`ROS2 Advertise for ${topicName}: Publisher state created.`);
                            ...
                            pluginsManager.addAction(hook, {
                                ...
                                action: async (selected_topic: SelectedTopic, message: any, webtype: any) => {
                                    const currentPublisherData = ros_publishers.get(topicName);
                                    if (!currentPublisherData) {
                                        console.warn(`ROS2 Publish action for ${topicName}: Publisher no longer exists.`);
                                        return;
                                    }
                                    try {
                                        const converted = UnifiedConverter.convertToROS2(message, webtype, rawType);
                                        const msg = new ROSLIB.Message(converted);
                                        currentPublisherData.topic.publish(msg);
                                    } catch (error) {
                                        console.error(`ROS2 Failed to publish message on ${topicName}:`, error);
                                    }
                                },
                                priority: 100,
                            });
                            console.log(`ROS2 Advertise for ${topicName}: Publish action registered for hook ${hook}.`);

                            // --- Critical Section End ---
                            console.log(`ROS2 Advertise for ${topicName}: Operation setup complete, returning true.`);
                            return true; // Indicate success

                        } catch (error) {
                            console.error(`ROS2 Advertise for ${topicName}: Error during operation:`, error);
                            // Clean up if advertise failed after creating state but before finishing
                            if (ros_publishers.has(topicName)) {
                                const hook = ros_publishers.get(topicName)?.hook;
                            ...
                            console.log(`ROS2 Advertise for ${topicName}: Operation finished, removed promise.`);
                        }
                    })();
            ...
            pluginsManager.addAction(unadvertise_hook, {
                ...
                action: async (topic: DatasourceTopic, ignoreCount: boolean = false) => {
                    ...
                    if (unadvertisingPromisesRef.current.has(topicName)) {
                        console.log(`ROS2 Unadvertise for ${topicName}: Operation already in progress, awaiting...`);
                    ...
                    const unadvertisePromise = (async (): Promise<void> => { // Return void
                        console.log(`ROS2 Unadvertise for ${topicName}: Starting operation (ignoreCount: ${ignoreCount}).`);
                        try {
                            ...
                            if (advertisingPromisesRef.current.has(topicName)) {
                                console.warn(`ROS2 Unadvertise for ${topicName}: Advertise operation still in progress. Awaiting completion before unadvertising.`);
                                try {
                                    await advertisingPromisesRef.current.get(topicName);
                                    console.log(`ROS2 Unadvertise for ${topicName}: Advertise operation completed. Proceeding with unadvertise.`);
                                } catch (advError) {
                                    console.warn(`ROS2 Unadvertise for ${topicName}: Advertise operation failed, proceeding with unadvertise cleanup anyway. Error:`, advError);
                                }
                            }
                            ...
                            if (!topic_and_counter) {
                                console.warn(`ROS2 Unadvertise for ${topicName}: Publisher not found within operation (potentially already unadvertised or advertise failed).`);
                                pluginsManager.removeAction(`${datasource_id}-${topicName}-publish`); // Cleanup hook just in case
                            ...
                            if (ignoreCount) {
                                ...
                                if (topic_and_counter.counter > 0) {
                                    ...
                                    console.warn(`ROS2 Unadvertise for ${topicName}: Counter was already zero or less before decrementing.`);
                                }
                            }
                            console.log(`ROS2 Unadvertise for ${topicName}: Count updated to ${topic_and_counter.counter}.`);

                            if (topic_and_counter.counter <= 0) {
                                console.log(`ROS2 Unadvertise for ${topicName}: Count is zero or less, proceeding with unadvertise.`);
                                try {
                                    topic_and_counter.topic.unadvertise();
                                    console.log(`ROS2 Unadvertise for ${topicName}: Unadvertise call sent.`);
                                } catch (unadvError) {
                                    console.error(`ROS2 Unadvertise for ${topicName}: Error calling topic.unadvertise:`, unadvError);
                                }

                                // Remove publisher state *after* attempting unadvertise
                                ros_publishers.delete(topicName);
                                pluginsManager.removeAction(topic_and_counter.hook);
                                console.log(`ROS2 Unadvertise for ${topicName}: Publisher state removed, action hook ${topic_and_counter.hook} removed.`);

                            } else {
                                console.log(`ROS2 Unadvertise for ${topicName}: Count is ${topic_and_counter.counter}, publisher remains active.`);
                            }
                            // --- Critical Section End ---
                            ...
                            console.log(`ROS2 Unadvertise for ${topicName}: Operation finished, removed promise.`);
                        }
                    })();

                    // Store the promise and await it to ensure the caller waits for completion
                    unadvertisingPromisesRef.current.set(topicName, unadvertisePromise);
        ...
        return () => {
            ...
            if (!connectionRef.current) {
                console.log("ROS2 Cleanup: Connection was not attempted, skipping cleanup.");
                return;
            }
            console.log("ROS2 Cleanup: Starting cleanup for datasource", datasource_id);
            ...
            subscribersRef.current.forEach((subscriber, topicName) => {
                try {
                    console.log(`ROS2 Cleanup: Unsubscribing from ${topicName}`);
                    subscriber.unsubscribe();
                } catch (e) { console.error(`ROS2 Cleanup: Error unsubscribing from ${topicName}:`, e); }
            });
            ...
            if (ros_publishers.size > 0) {
                console.warn(`ROS2 Cleanup: ${ros_publishers.size} publishers still in map during provider cleanup. This might indicate components did not unadvertise correctly.`);
