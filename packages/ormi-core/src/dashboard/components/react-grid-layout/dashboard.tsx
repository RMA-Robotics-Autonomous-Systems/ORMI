...
const Dashboard = () => {
  ...
  const exploseLayout = useCallback(
    (type: "custom" | "rows" | "columns" | "masonry" | "single" = "custom") => {
      if (locked) {
        showToast(
      ...
      function getOptimalMatrix(
        ...
        const sorted_distance_matrixes = Array.from(distance_matrixes_map).sort(
          (a, b) => {
            if (a[1] === b[1]) {
              const ma = availables_matrixes.get(a[0]);
              const mb = availables_matrixes.get(b[0]);
              return (
                (ma ? ma.cols * ma.rows : 0) - (mb ? mb.cols * mb.rows : 0)
              );
            }
            return b[1] - a[1];
          },
      ...
      const max_number_of_rows =
        (typeof window !== "undefined" ? window.innerHeight * 0.9 : 900) /
        row_size_px;
          ...
          <ButtonHolderProvider>
            ...
            >
              ...
              {!locked && (
                <WidgetCard
                  ...
                  onValidate={(widget_def, settings) => {
                    handleSaveWidget(widget.box_id, widget_def, settings);
                  }}
                />
              )}
              ...
              {!locked && (
                <Button
                  variant="destructive"
                  onClick={() => handleRemoveBoxClick(widget.box_id)}
                >
                  <XIcon />
                </Button>
              )}
