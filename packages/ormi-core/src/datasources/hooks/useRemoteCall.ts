...
>(
  ...
  const handleRef = useRef<RemoteCallHandle<TFeedback, TResponse> | null>(null);
  ...
  const execute = useCallback(
    async (
      ...
      try {
        ...
        const handle = await pluginsManager.applyFilterAsync<RemoteCallHandle<
          TFeedback,
          TResponse
        ...
        statusUnsubscribeRef.current = handle.onStatusChange((newStatus) => {
          setStatus(newStatus);
        });
...
function useAvailableRemoteCalls(filter?: UseAvailableRemoteCallsFilter) {
  ...
  const filteredCalls = useMemo(() => {
    ...
    if (filter?.datasource_id) {
      result = result.filter((c) => c.datasource_id === filter.datasource_id);
    }
