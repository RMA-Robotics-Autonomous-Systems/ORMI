...
interface RemoteCallHandle<TFeedback = unknown, TResult = unknown> {
   ...
   * @returns Unsubscribe function
   */
  onStatusChange: (callback: (status: RemoteCallStatus) => void) => () => void;
...
class RemoteCallFilter {
  ...
  filter(call: RemoteCallDefinition): boolean {
    ...
    if (this.rawRequestType && !this.rawRequestType.test(call.rawRequestType)) {
      failures.push(true);
    }
    ...
    if (this.datasource_id && !this.datasource_id.test(call.datasource_id)) {
      failures.push(true);
    }
