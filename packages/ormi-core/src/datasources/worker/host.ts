...
export class WorkerDatasourceHost<Settings = DatasourceProviderSettings> {
  ...
  private registerEventHandlers(): void {
    ...
    const statusUnsub = this.rpc.onEvent("remote-call-status", (payload) => {
      const handlers = this.remoteCallStatusHandlers.get(payload.callId);
      if (!handlers) return;
    ...
    const feedbackUnsub = this.rpc.onEvent(
      ...
      (payload) => {
        const handlers = this.remoteCallFeedbackHandlers.get(payload.callId);
        if (!handlers) return;
        handlers.forEach((handler) => handler(payload.feedback));
      },
    );

    const resultUnsub = this.rpc.onEvent("remote-call-result", (payload) => {
      const handler = this.remoteCallResultHandlers.get(payload.callId);
      if (handler) {
  ...
  private createRemoteCallHandle(
    ...
    return {
      ...
      onFeedback: (callback) => {
        const set =
          this.remoteCallFeedbackHandlers.get(handleWire.callId) ?? new Set();
      ...
      onStatusChange: (callback) => {
        const set =
          this.remoteCallStatusHandlers.get(handleWire.callId) ?? new Set();
        set.add(callback);
        ...
        return () => {
          const current = this.remoteCallStatusHandlers.get(handleWire.callId);
          if (!current) return;
