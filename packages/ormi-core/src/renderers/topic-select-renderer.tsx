...
const TopicSelectRenderer = (props: ControlProps) => {
    ...
    const dataRequirements = uischema.options?.dataRequirements as DataRequirements | undefined;

    // Legacy support: detect old-style options
    const isLegacyMode = !dataRequirements && (
    ...
    const getTypeInfo = () => {
        ...
        return (
            <div className="flex gap-1 mt-1">
                ...
                {selectedTopic.rawType && selectedTopic.rawType !== selectedTopic.type && (
        ...
        <div className="space-y-2">
            ...
            {process.env.NODE_ENV === 'development' && (
                <div className="text-xs text-muted-foreground p-2 bg-muted rounded">
                    {dataRequirements ? (
                        <span><strong>Requirements:</strong> {dataRequirements.accepts.join(', ')}</span>
                    ) : (
                        <span><strong>No Requirements:</strong> Any topic can be selected</span>
                    )}
                </div>
            )}
...
const topicSelectTester = rankWith(
    10,
    and(
        isControl,
        uiTypeIs('TopicSelect')
    )
);
