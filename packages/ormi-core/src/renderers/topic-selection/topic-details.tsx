                    ...
                    <CardContent className="space-y-3">
                        <div className="text-sm">
                            <div>
                                ...
                                <p className="font-medium">{topic.source.title}</p>
                            </div>
                        </div>
                        ...
                        {requirements && analysis && (
                            <div className="space-y-2">
                                ...
                                <div className={cn(
                                    ...
                                    analysis.isCompatible ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"
                                )}>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className={cn(
                                            "text-sm font-medium",
                                            analysis.isCompatible ? "text-green-700" : "text-red-700"
                                        )}>
                                    ...
                                    {analysis.compatibleProperties.length > 0 && !analysis.directMatch && (
                                        <p className="text-xs text-green-600">
                                            {analysis.compatibleProperties.length} compatible properties found
                                        </p>
                                    )}
                    ...
                    <CardContent className="pt-6">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                max="1000"
                                value={bufferSize}
                                onChange={(e) => onBufferSizeChange(parseInt(e.target.value) || 1)}
                                className="w-full"
                            />
                            <p className="text-xs text-muted-foreground">
                                Number of messages to keep in buffer for this topic
                            </p>
                        </div>
                    </CardContent>
                </Card>
                {/* Property Trees */}
                {propertyTree && tabInfo && (propertyTree.hasWebappData || propertyTree.hasRawData) && (
                    <Card className="flex-1 flex flex-col min-h-0">
                        ...
                        <CardContent className="flex-1 flex flex-col min-h-0">
                            <Tabs value={activeTab} onValueChange={onTabChange as (value: string) => void} className="flex-1 flex flex-col">
                                ...
                                <TabsContent value="webapp" className="flex-1 mt-4">
                                    <ScrollArea className="h-full">
                                        <PropertyTree
                                            nodes={propertyTree.webapp}
                                            selectedProperty={selectedPropertySource === 'webapp' ? selectedProperty : null}
                                            onPropertySelect={(path) => onPropertySelect(path, 'webapp')}
                                ...
                                <TabsContent value="raw" className="flex-1 mt-4">
                                    <ScrollArea className="h-full">
                                        <PropertyTree
                                            ...
                                            selectedProperty={selectedPropertySource === 'raw' ? selectedProperty : null}
                                            onPropertySelect={(path) => onPropertySelect(path, 'raw')}
                                            requirements={requirements}
                                        />
                                    </ScrollArea>
                                </TabsContent>
        ...
        return (
            <div key={node.path} className="select-none">
                ...
                >
                    ...
                    {node.type && (
                        <Badge
                            variant={node.isCompatible ? "default" : "secondary"}
                            className="text-xs"
                        >
                            {node.type}
                        </Badge>
