...
import { DatasourceTopic, SelectedTopic } from '../../datasources/datasource-interface';
...
export const TopicSelectionDialog: React.FC<TopicSelectionDialogProps> = ({
    ...
    const [state, dispatch] = useReducer(topicSelectionReducer, initialTopicSelectionState);
        ...
        const loadTopics = async () => {
            ...
            try {
                ...
                if (initialValue) {
                    ...
                    if (matchingTopic) {
                        dispatch({ type: 'SELECT_TOPIC', topic: matchingTopic });
                    }
                }
    ...
    const analyzeAllTopics = async (topics: DatasourceTopic[], reqs?: DataRequirements) => {
        ...
        dispatch({ type: 'SET_COMPATIBILITY_ANALYSIS', analysis: analysisResults });
            ...
            if (matchingTopic) {
                ...
                if (propertyTree) {
                    ...
                    if (propertyTree.webapp) {
                        foundInWebapp = findPropertyNodeByPath(propertyTree.webapp, propertyPath) !== null;
                    }
                    ...
                    dispatch({ type: 'SELECT_PROPERTY', path: propertyPath, source: propertySource });
    ...
    const canConfirm = state.selectedTopic && (
        !requirements ||
        state.compatibilityAnalysis.get(`${state.selectedTopic.topic}@${state.selectedTopic.source.id}`)?.isCompatible
    );
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent size='large'>
                <DialogHeader>
                    ...
                    {!requirements && (
                        <p className="text-sm text-muted-foreground">
                            No data requirements specified - any topic can be selected
                        </p>
                    )}
                    <div className="flex items-center gap-2 mt-2">
                        ...
                        <Button
                            variant="outline"
                            size="sm"
                            onClick={() => dispatch({ type: 'TOGGLE_COMPATIBILITY_FILTER' })}
                            className={state.showOnlyCompatible ? 'bg-primary text-primary-foreground' : ''}
                ...
                <div className="flex gap-4 overflow-hidden">
                    ...
                    <div className="w-96 flex flex-col border rounded-lg" style={{ height: '50dvh', overflowY: 'auto' }}>
                        <TopicBrowser
                    ...
                    <div className="flex-1 flex flex-col" style={{ height: '50dvh', overflowY: 'auto' }}>
                        <TopicDetails
                            topic={state.selectedTopic}
                            analysis={state.selectedTopic ?
                                state.compatibilityAnalysis.get(`${state.selectedTopic.topic}@${state.selectedTopic.source.id}`) || null :
                                null
                            }
                            propertyTree={state.selectedTopic ?
                                state.propertyTrees.get(`${state.selectedTopic.topic}@${state.selectedTopic.source.id}`) || null :
                                null
                            }
                            selectedProperty={state.selectedProperty}
                            selectedPropertySource={state.selectedPropertySource}
                            onPropertySelect={handlePropertySelect}
                            activeTab={state.activeTab}
                            onTabChange={(tab: 'webapp' | 'raw') => dispatch({ type: 'SWITCH_TAB', tab })}
                            bufferSize={state.bufferSize}
                            onBufferSizeChange={(size: number) => dispatch({ type: 'SET_BUFFER_SIZE', size })}
                            requirements={requirements}
                        />
                    </div>
                </div>
                ...
                <DialogFooter>
                    <div className="flex items-center justify-between w-full">
                        ...
                        <div className="flex-1 text-sm text-muted-foreground">
                            {state.selectedTopic && (
                                <span>
                                    Selected: <strong>{state.selectedTopic.topic}</strong>
                                    {state.selectedProperty && (
                                        <span> → <strong>{state.selectedProperty}</strong> ({state.selectedPropertySource})</span>
                                    )}
                                </span>
                            )}
                        </div>
