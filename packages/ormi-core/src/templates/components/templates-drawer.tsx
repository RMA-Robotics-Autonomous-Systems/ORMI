...
import { Template, WidgetTemplate, DatasourceTemplate } from "../templates-types";
...
import { DatasourceDefinition, DatasourceProviderSettings } from "../../datasources";
...
export function WidgetTemplateDrawer(props: WidgetTemplateDrawerProps) {
    const { templates, removeTemplate, addWidget, addDatasource, updateTemplate } = props;
    ...
    const yourWidgetTemplates = filteredTemplates.filter(([key, template]) =>
        template.type === 'widget' && template.yours
    ).reduce((acc, [key, template]) => {
        acc.set(key, template as WidgetTemplate);
        return acc;
    }, new Map<string, WidgetTemplate>());

    const publicWidgetTemplates = filteredTemplates.filter(([key, template]) =>
        template.type === 'widget' && !template.yours
    ).reduce((acc, [key, template]) => {
        acc.set(key, template as WidgetTemplate);
        return acc;
    ...
    const yourDatasourceTemplates = filteredTemplates.filter(([key, template]) =>
    ...
    const publicDatasourceTemplates = filteredTemplates.filter(([key, template]) =>
        template.type === 'datasource' && !template.yours
    ).reduce((acc, [key, template]) => {
        acc.set(key, template as DatasourceTemplate);
        return acc;
                ...
                <div className="p-4 space-y-4">
                    ...
                    <Tabs defaultValue="widgets" className="w-full">
                        ...
                        <TabsContent value="widgets">
                            <Accordion type="single" defaultValue="user-widgets" collapsible>
                                <AccordionItem value="user-widgets">
                                    ...
                                    <AccordionContent>
                                        {yourWidgetTemplates.size === 0 ? (
                                            <p className="text-sm text-muted-foreground">No widget templates found</p>
                                        ) : (
                                            Array.from(yourWidgetTemplates.entries()).map(([key, template]) => (
                                                <TemplateComponent
                                                    key={key}
                                                    templateId={key}
                                ...
                                <AccordionItem value="public-widgets">
                                    ...
                                    <AccordionContent>
                                        {publicWidgetTemplates.size === 0 ? (
                                            <p className="text-sm text-muted-foreground">No public widget templates found</p>
                                        ) : (
                                            Array.from(publicWidgetTemplates.entries()).map(([key, template]) => (
                                                <TemplateComponent
                                                    key={key}
                                                    templateId={key}
                                                    availableWidgets={availableWidgets}
                        ...
                        <TabsContent value="datasources">
                            <Accordion type="single" defaultValue="user-datasources" collapsible>
                                <AccordionItem value="user-datasources">
                                    <AccordionTrigger>
                                        Your datasource templates ({yourDatasourceTemplates.size})
                                    </AccordionTrigger>
                                    <AccordionContent>
                                        {yourDatasourceTemplates.size === 0 ? (
                                            <p className="text-sm text-muted-foreground">No datasource templates found</p>
                                        ) : (
                                            Array.from(yourDatasourceTemplates.entries()).map(([key, template]) => (
                                ...
                                <AccordionItem value="public-datasources">
                                    <AccordionTrigger>
                                        Public datasource templates ({publicDatasourceTemplates.size})
                                    </AccordionTrigger>
                                    <AccordionContent>
                                        {publicDatasourceTemplates.size === 0 ? (
                                            <p className="text-sm text-muted-foreground">No public datasource templates found</p>
                                        ) : (
                                            Array.from(publicDatasourceTemplates.entries()).map(([key, template]) => (
                                                <DatasourceTemplateComponent
                                                    key={key}
                                                    templateId={key}
                                                    availableDatasources={availableDatasources}
