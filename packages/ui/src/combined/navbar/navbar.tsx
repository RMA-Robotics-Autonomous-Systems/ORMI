...
export const NavBar = () => {
  ...
  const sortNavItems = (
    items: Map<string, { component: React.ReactNode; priority: number }>,
  ) => {
      ...
      <NavigationMenu className="shadow-md w-full p-1">
            ...
            <SheetContent side="left" className="w-[300px] sm:w-[350px] p-0">
              ...
              <div className="flex flex-col space-y-4 p-4 overflow-y-auto h-full">
                    ...
                    <div className="overflow-x-auto pb-2">
                      <div className="grid grid-flow-col auto-cols-max gap-2 min-w-full">
                        {sortedCenter.map(([key, value]) => (
                          <div
                            key={key}
                            className="rounded-md hover:bg-muted p-2 flex items-center"
                          >
                            {value.component}
                          </div>
                        ))}
                ...
                {sortedRight.length > 0 && (
                  <div className="space-y-3">
                    {(sortedLeft.length > 0 || sortedCenter.length > 0) && (
                      <div className="h-px bg-border my-2" />
                    )}
                    <h3 className="font-medium text-sm text-muted-foreground px-2">
                      Account
        ...
        <div className="hidden md:flex w-full justify-between">
          <NavigationMenuList key="left">
            {sortedLeft.map(([key, value]) => (
              <NavigationMenuItem style={{ height: "40px" }} key={key}>
                {value.component}
              </NavigationMenuItem>
            ))}
          </NavigationMenuList>
