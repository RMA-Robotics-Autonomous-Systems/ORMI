...
const predefinedThemes = [
	...
	},
	{
		name: "Slate",
		light: {
			primary: "oklch(0.151 0.008 256.848)",
			secondary: "oklch(0.969 0.002 256.848)",
			accent: "oklch(0.923 0.006 256.848)",
		},
		dark: {
			primary: "oklch(0.984 0.003 256.848)",
			secondary: "oklch(0.277 0.011 256.848)",
			accent: "oklch(0.277 0.011 256.848)",
		},
	},
	{
		name: "Stone",
		light: {
			primary: "oklch(0.154 0.003 49.748)",
			secondary: "oklch(0.969 0.003 49.748)",
			accent: "oklch(0.924 0.007 49.748)",
		},
		dark: {
			primary: "oklch(0.984 0.002 49.748)",
			secondary: "oklch(0.278 0.006 49.748)",
			accent: "oklch(0.278 0.006 49.748)",
		},
	},
	{
		name: "Gray",
	...
	},
	{
		name: "Rose",
		light: {
			primary: "oklch(0.588 0.229 7.36)",
			secondary: "oklch(0.97 0 0)",
			accent: "oklch(0.922 0 0)",
		},
		dark: {
			primary: "oklch(0.651 0.22 7.36)",
			secondary: "oklch(0.269 0 0)",
			accent: "oklch(0.269 0 0)",
		},
	},
	{
		name: "Orange",
		light: {
			primary: "oklch(0.654 0.218 70.67)",
			secondary: "oklch(0.97 0 0)",
			accent: "oklch(0.922 0 0)",
		},
		dark: {
			primary: "oklch(0.738 0.21 70.67)",
			secondary: "oklch(0.269 0 0)",
			accent: "oklch(0.269 0 0)",
		},
	},
	{
		name: "Green",
		light: {
			primary: "oklch(0.546 0.176 145.97)",
			secondary: "oklch(0.97 0 0)",
			accent: "oklch(0.922 0 0)",
		},
		dark: {
			primary: "oklch(0.629 0.17 145.97)",
			secondary: "oklch(0.269 0 0)",
			accent: "oklch(0.269 0 0)",
		},
	},
	{
		name: "Blue",
		light: {
			primary: "oklch(0.571 0.191 252.07)",
			secondary: "oklch(0.97 0 0)",
			accent: "oklch(0.922 0 0)",
		},
		dark: {
			primary: "oklch(0.664 0.185 252.07)",
			secondary: "oklch(0.269 0 0)",
			accent: "oklch(0.269 0 0)",
		},
	},
	{
		name: "Yellow",
		light: {
			primary: "oklch(0.832 0.182 85.04)",
			secondary: "oklch(0.97 0 0)",
			accent: "oklch(0.922 0 0)",
		},
		dark: {
			primary: "oklch(0.888 0.175 85.04)",
			secondary: "oklch(0.269 0 0)",
			accent: "oklch(0.269 0 0)",
		},
	},
	{
		name: "Violet",
		light: {
			primary: "oklch(0.627 0.198 293.71)",
			secondary: "oklch(0.97 0 0)",
			accent: "oklch(0.922 0 0)",
		},
		dark: {
			primary: "oklch(0.734 0.193 293.71)",
			secondary: "oklch(0.269 0 0)",
			accent: "oklch(0.269 0 0)",
		},
	},
...
export function ThemeConfigurator({
	...
	const applyTheme = useCallback(
		(config: ThemeConfig) => {
			...
			let darkStyleElement =
				document.getElementById("dynamic-dark-theme");
			if (!darkStyleElement) {
	...
	const updateBorderIntensity = (intensity: number) => {
		...
		const match = currentBorder.match(
			/oklch\(\s*([^\s]+)\s+([^\s]+)\s+([^\s)]+)\s*\)/,
		);
		if (match && match[1] && match[2] && match[3]) {
			...
			const adjustedL = Math.max(
				0,
				Math.min(1, l * (2 - intensity / 50)),
			); // Scale based on intensity (reversed)
	...
	const oklchToHex = (oklchValue: string): string => {
		...
		try {
			...
			if (isNaN(l) || isNaN(c) || isNaN(h)) {
				console.warn("Invalid OKLCH values after parsing:", {
					l,
					c,
					h,
					original: oklchValue,
			...
			if (!hexColor) {
				console.warn(
					"Failed to format HEX from OKLCH:",
					oklchValue,
					colorObject,
				);
						...
						{predefinedThemes.map((theme) => {
							...
							return (
								<Button
									key={theme.name}
									variant={
										currentConfig.name === theme.name
											? "default"
											: "outline"
									}
									size="sm"
									onClick={() =>
										applyPredefinedTheme(theme.name)
									}
					...
					<div className="grid grid-cols-3 gap-2">
						{[
							...
							<Button
								key={option.value}
								variant={
									currentConfig.light.radius === option.value
										? "default"
										: "outline"
								}
								size="sm"
				...
				<div className="space-y-3">
					...
					<div className="space-y-2">
						<Slider
							...
							onValueChange={(value) =>
								updateShadowIntensity(value[0] ?? 50)
							}
							max={100}
							min={0}
							step={1}
				...
				<div className="space-y-3">
					...
					<div className="space-y-2">
						<Slider
							...
							onValueChange={(value) =>
								updateBorderIntensity(value[0] ?? 50)
							}
							max={100}
							min={0}
							step={1}
	...
	const RadiusInput = () => {
		...
		return (
			<div className="space-y-3">
				<Label className="text-sm font-medium">
					Border Radius (applies to both themes)
				</Label>
				<div className="grid grid-cols-3 gap-2">
		...
		<Dialog>
			...
			<DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
				<DialogHeader>
					...
					<DialogDescription>
						Customize your theme colors and save/load
						configurations. Changes are applied live to see the
						effects immediately.
					</DialogDescription>
				</DialogHeader>
					...
					{!isAdvancedMode ? (
						...
						<Tabs
							...
							onValueChange={(value: string) =>
								setPreviewMode(value as "light" | "dark")
							}
						>
											...
											<div className="flex flex-col gap-2">
												<Button
													onClick={() => {
														...
														alert(
															"Theme configuration saved to localStorage.",
														);
													}}
													variant="outline"
													size="sm"
												...
												<Button
													onClick={() => {
														...
														if (loadedConfig) {
															...
															alert(
																"No valid theme configuration found in localStorage.",
															);
														}
													}}
													variant="outline"
												...
												<Button
													onClick={() => {
														...
														alert(
															"Theme configuration cleared from localStorage.",
														);
													}}
													variant="outline"
													size="sm"
