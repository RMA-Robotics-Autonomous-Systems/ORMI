...
const pendingAdvertiseOps = new Map<
  string,
  { topic: DatasourceTopic; resolve: (success: boolean) => void }
>();
const pendingPublishOps: Array<{
...
const connect = (
  ...
  ws.addEventListener("error", () => {
    ...
    if (errorHandler) {
      errorHandler.handle(
        createConnectionError("Foxglove WebSocket error", settings?.id, {
          url: settings?.url,
        }),
      );
    }
  ...
  newServices.forEach((service) => {
    ...
    if (service.request?.schema) {
      try {
        ...
        writersByServiceId.set(service.id, new MessageWriter(parsedRequest));
      } catch {
        // ignore
      }
    }
    if (service.response?.schema) {
      try {
        ...
        readersByServiceId.set(service.id, new MessageReader(parsedResponse));
      } catch {
        // ignore
      }
    }
...
>(
  ...
  },
  {
    ...
    subscribe: async (topic: SelectedTopic) => {
      if (!client || !connected) {
        pendingSubscriptions.set(topic.topic, {
          topic: topic.topic,
          count: (pendingSubscriptions.get(topic.topic)?.count ?? 0) + 1,
        });
        return;
      }
    ...
    listTypes: async (webtypes?: string[]) => {
      ...
      let uniqueSchemas = Array.from(new Set([...schemas, ...interfaceList]));
