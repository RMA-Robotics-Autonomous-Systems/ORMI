  ...
  private registerPublishAction(publisher: Publisher): void {
    ...
    this.pluginsManager.addAction(hook, {
      ...
      action: async (selectedTopic: any, message: any, webtype: any) => {
        try {
          const currentPublisher = this.publishers.get(publisher.channelId);
          if (!currentPublisher) {
            console.warn(`Publisher for ${publisher.topic} no longer exists`);
            return;
          }
    ...
    try {
      ...
      if (publisher.count <= 0) {
        ...
        try {
          ...
          console.error(`Error unadvertising channel ${channelId}:`, error);
        }
