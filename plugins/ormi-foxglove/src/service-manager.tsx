...
const ServiceManager: React.FC<ServiceManagerProps> = ({
	...
	const handleAdvertiseServices = useCallback(
		(newServices: Service[]) => {
			setServices((prev) => {
				...
				for (const service of newServices) {
					...
					if (service.request?.schema) {
						try {
							const parsedRequest = parse(
								service.request.schema,
								{
									ros2: true,
								},
							);
							writersRef.current.set(
								service.id,
								new MessageWriter(parsedRequest),
							);
						} catch (err) {
							console.error(
								`Failed to create writer for service ${service.name}:`,
								err,
							);
						}
					}
					if (service.response?.schema) {
						try {
							...
							readersRef.current.set(
								service.id,
								new MessageReader(parsedResponse),
							);
						} catch (err) {
							console.error(
								`Failed to create reader for service ${service.name}:`,
								err,
							);
						}
					}
	...
	const handleUnadvertiseServices = useCallback(
	...
	const handleServiceCallResponse = useCallback(
		(response: ServiceCallResponse) => {
			const pending = pendingCallsRef.current.get(response.callId);
			if (!pending) {
				console.warn(
					`Received response for unknown call ID: ${response.callId}`,
				);
				return;
			}
	...
	const handleServiceCallFailure = useCallback(
		(failure: ServiceCallFailure) => {
			const pending = pendingCallsRef.current.get(failure.callId);
			if (!pending) {
				console.warn(
					`Received failure for unknown call ID: ${failure.callId}`,
				);
				return;
			}
	...
	const serviceToRemoteCallDefinition = useCallback(
		(service: Service): RemoteCallDefinition => {
			...
			if (service.request?.schema) {
				try {
					...
					console.warn(
						`Failed to parse request schema for ${service.name}:`,
						err,
					);
				}
			}
			...
			if (service.response?.schema) {
				try {
					...
					console.warn(
						`Failed to parse response schema for ${service.name}:`,
						err,
					);
				}
			}
	...
	const executeRemoteCall = useCallback(
		async (
			...
			if (!writer) {
				throw new Error(
					`No writer available for service: ${definition.name}`,
				);
			}
			if (!reader) {
				throw new Error(
					`No reader available for service: ${definition.name}`,
				);
			}
	...
	useEffect(() => {
		...
		const advertiseHandler = (newServices: Service[]) =>
			handleAdvertiseServices(newServices);
		const unadvertiseHandler = (removedIds: number[]) =>
			handleUnadvertiseServices(removedIds);
		const responseHandler = (response: ServiceCallResponse) =>
			handleServiceCallResponse(response);
		const failureHandler = (failure: ServiceCallFailure) =>
			handleServiceCallFailure(failure);
		...
		client,
		handleAdvertiseServices,
		handleUnadvertiseServices,
		handleServiceCallResponse,
		handleServiceCallFailure,
	]);
	...
	useEffect(() => {
		...
		const remoteCallDefs = Array.from(services.values()).map(
	...
	useEffect(() => {
		...
		pluginsManager.addFilter(PluginsHooks.AVAILABLE_REMOTE_CALLS, {
			...
			filter: (calls: RemoteCallDefinition[]) => {
				const foxgloveCalls = Array.from(services.values()).map(
					serviceToRemoteCallDefinition,
				);
