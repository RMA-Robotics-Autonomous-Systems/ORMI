...
const TransformTreeManager: React.FC<TransformTreeManagerProps> = ({ children, settings }) => {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            (settings.transformTreeTopics || []).forEach(async (topic) => {
                ...
                try {
                    await pluginsManager.doAction(`${datasource_id}-unsubscribe`, datasourceTopic, true);
                } catch (error) {
    ...
    }, [settings.enable, settings.id, settings.transformTreeTopics, pluginsManager, datasource_id]);
