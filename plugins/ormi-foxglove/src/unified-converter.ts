      ...
      conversions: {
        "geometry_msgs/msg/Twist": {
          toRos2: (data: Movement) => {
            ...
            if (!data) {
              throw new Error("Movement data is null or undefined");
            }
          ...
          toRos2: (data: GeolocationPosition) => ({
            ...
            header: {
              stamp: {
                ...
                nanosec: Math.floor((data.timestamp % 1000) * 1e6),
              },
            },
          }),
          fromRos2: (data: any) => {
            const covariance = Array.isArray(data.position_covariance)
        ...
        "nav_msgs/msg/Path": {
            ...
            poses: data.poses.map((pose: PoseStamped) => ({
              header: {
                stamp: {
                  ...
                  nanosec: Math.floor((pose.timestamp % 1) * 1e9),
                },
                frame_id: "",
              },
              pose: {
          ...
          fromRos2: (data: any) => {
            const timestamp = data.header?.stamp
              ? data.header.stamp.sec + data.header.stamp.nanosec / 1e9
              : Date.now() / 1000;
      ...
      conversions: {
        "sensor_msgs/msg/PointCloud2": {
          ...
          fromRos2: (data): PointsCloud => {
            ...
            const intensityOffset =
              fieldMap.intensity?.offset ?? fieldMap.reflectivity?.offset;
            const intensityDatatype =
              fieldMap.intensity?.datatype ?? fieldMap.reflectivity?.datatype;
            const rgbOffset = fieldMap.rgb?.offset ?? fieldMap.rgba?.offset;
            const rgbDatatype =
            ...
            const readRgb = (
              ...
              if (datatype === 7) {
                const floatValue = view.getFloat32(offset, littleEndian);
              ...
              if (datatype === 6 || datatype === 5) {
                const uintValue = view.getUint32(offset, littleEndian);
                return unpackRgb(uintValue);
              }
            ...
            for (let i = 0; i < totalPoints; i++) {
              ...
              try {
                ...
                if (!isNaN(x) && !isNaN(y) && !isNaN(z)) {
                  ...
                  if (rgbOffset !== undefined && rgbDatatype !== undefined) {
                    const color = readRgb(
                      dataView,
                  ...
                  if (
                    ...
                    intensities[validPointCount] = normalized;
                  }
        ...
        "livox_ros_driver2/msg/CustomMsg": {
          ...
          fromRos2: (data): PointsCloud => {
            ...
            const finalPoints = packedPoints.subarray(0, validPointCount * 3);
            const finalColors =
              validPointCount > 0 && data.points[0]?.reflectivity !== undefined
                ? packedColors.subarray(0, validPointCount * 3)
                : undefined;
            const finalIntensities =
              validPointCount > 0 && data.points[0]?.reflectivity !== undefined
