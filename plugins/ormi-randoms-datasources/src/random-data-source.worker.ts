    ...
    switch (topic.type) {
      ...
      case "IMU":
        return setInterval(() => {
          ...
          const imuData = {
            linear_acceleration: {
              x: stepAmplitude * Math.sin(2 * Math.PI * stepFrequency * time),
              y: Math.abs(
                stepAmplitude * Math.sin(4 * Math.PI * stepFrequency * time),
              ),
              z:
                stepAmplitude *
      ...
      case "PointsCloud": {
        ...
        return setInterval(() => {
          ...
          for (let i = 0; i < numPoints; i++) {
            ...
            positions[idx] = basePoint.x + Math.sin(t + phase.x) * amplitude;
            positions[idx + 1] =
