...
export class RestBagClient {
	...
	async getBag(name: string): Promise<BagInfo[]> {
		const response = await fetch(
			`${this.baseUrl}/bags/${encodeURIComponent(name)}`,
		);

		if (!response.ok) {
			throw new Error(
				`Failed to fetch bag "${name}": ${response.statusText}`,
			);
		}
	...
	async deleteBag(
		...
		const response = await fetch(
			`${this.baseUrl}/bags/${encodeURIComponent(name)}?multiple=${multiple}`,
			{
				method: "DELETE",
			},
		);
		...
		if (!response.ok) {
			throw new Error(
				`Failed to delete bag "${name}": ${response.statusText}`,
			);
		}
	...
	async downloadBag(name: string, multiple: boolean = false): Promise<Blob> {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to download bag "${name}": ${response.statusText}`,
			);
		}
	...
	async startCompression(
		...
		const response = await fetch(
			`${this.baseUrl}/bags/${encodeURIComponent(name)}/compress?multiple=${multiple}`,
			{
				method: "POST",
			},
		);
		...
		if (!response.ok) {
			throw new Error(
				`Failed to start compression for bag "${name}": ${response.statusText}`,
			);
		}
	...
	async getCompressionStatus(taskId: string): Promise<CompressionStatus> {
		const response = await fetch(
			`${this.baseUrl}/bags/compress/${encodeURIComponent(taskId)}/status`,
		);

		if (!response.ok) {
			throw new Error(
				`Failed to get compression status for task "${taskId}": ${response.statusText}`,
			);
		}
	...
	async listCompressionTasks(): Promise<CompressionTasksList> {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to list compression tasks: ${response.statusText}`,
			);
		}
	...
	async getDownloadInfo(downloadId: string): Promise<DownloadInfo> {
		const response = await fetch(
			`${this.baseUrl}/bags/download/${encodeURIComponent(downloadId)}/info`,
		);

		if (!response.ok) {
			throw new Error(
				`Failed to get download info for "${downloadId}": ${response.statusText}`,
			);
		}
	...
	): Promise<RecordingResponse | RecordingError> {
	...
	async getRecording(
		recordingId: string,
	): Promise<RecordingStatus | RecordingError> {
		const response = await fetch(
			`${this.baseUrl}/recordings/${encodeURIComponent(recordingId)}`,
		);

		if (!response.ok) {
			throw new Error(
				`Failed to fetch recording: ${response.statusText}`,
			);
		}
	...
	async pausePlayback(playId: string): Promise<PlayerActionResponse> {
		const response = await fetch(
			`${this.baseUrl}/player/${encodeURIComponent(playId)}/pause`,
			{
				method: "POST",
			},
		);
	...
	async resumePlayback(playId: string): Promise<PlayerActionResponse> {
		const response = await fetch(
			`${this.baseUrl}/player/${encodeURIComponent(playId)}/resume`,
			{
				method: "POST",
			},
		);
	...
	async stopPlayback(playId: string): Promise<PlayerActionResponse> {
		const response = await fetch(
			`${this.baseUrl}/player/${encodeURIComponent(playId)}/stop`,
			{
				method: "POST",
			},
		);
	...
	async getPlayerStatus(playId: string): Promise<PlayerStatusResponse> {
		const response = await fetch(
			`${this.baseUrl}/player/${encodeURIComponent(playId)}`,
		);

		if (!response.ok) {
			throw new Error(
				`Failed to get player status: ${response.statusText}`,
			);
		}
	...
	async getAvailableTopics(): Promise<Topic[]> {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to fetch available topics: ${response.statusText}`,
			);
		}
		...
		const topics = (await response.json()) as {
