...
import { TableBody, TableRow, TableCell, TableHeader, TableHead, Table } from "@workspace/ui/components/table";
        ...
        <Dialog>
            ...
            <DialogContent className="sm:max-w-[800px] max-h-[80vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {bag.path ? `File: ${bag.path}` : 'Details about the selected ROS bag file'}
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <TableBody>
                            {displayableEntries.map(([key, value]) => (
                                <TableRow key={key}>
                                    ...
                                    <TableCell>
                                        {key.includes('time') ? formatDate(value) : String(value)}
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                            ...
                            <Table>
                                <TableBody>
                                    {bag.meta.storage_identifier && (
                                        <TableRow>
                                            <TableCell className="font-medium">Storage Format</TableCell>
                                            <TableCell>{bag.meta.storage_identifier}</TableCell>
                                        </TableRow>
                                    )}
                                    {bag.meta.duration && (
                                        <TableRow>
                                            ...
                                            <TableCell>
                                                {(bag.meta.duration.nanoseconds / 1000000000).toFixed(2)} seconds
                                            </TableCell>
                                        </TableRow>
                                    )}
                                    {bag.meta.starting_time && (
                                        <TableRow>
                                            <TableCell className="font-medium">Starting Time</TableCell>
                                            <TableCell>
                                                {new Date(bag.meta.starting_time.nanoseconds_since_epoch / 1000000).toLocaleString()}
                                            </TableCell>
                                        </TableRow>
                                    )}
                                    {bag.meta.message_count && (
                                        <TableRow>
                                            ...
                                            <TableCell>{bag.meta.message_count.toLocaleString()}</TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
                            </Table>
                ...
                {bag.meta?.topics_with_message_count && (
                    <>
                        ...
                        <div className="space-y-2">
                            <h3 className="text-lg font-medium">Topics ({bag.meta.topics_with_message_count.length})</h3>
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                ...
                                <TableBody>
                                    {bag.meta.topics_with_message_count.map((topic, index) => (
                                        <TableRow key={index}>
                                            <TableCell className="font-medium">{topic.topic_metadata.name}</TableCell>
                                            <TableCell>{topic.topic_metadata.type}</TableCell>
