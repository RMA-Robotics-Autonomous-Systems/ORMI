		...
		const negotiate = async () => {
			try {
				...
				if (cleanupRef.current) {
					console.log(
						"Component cleanup in progress, aborting negotiation",
					);
					return;
				}
				...
				await new Promise<void>((resolve) => {
					const timeout = setTimeout(() => {
						console.warn(
							"ICE gathering timed out, proceeding anyway",
						);
						resolve();
					}, 5000);
				...
				if (cleanupRef.current) {
					console.log(
						"Cleanup before setting remote description, aborting",
					);
					return;
				}
				...
				console.log(
					"Remote description set, WebRTC negotiation complete",
				);
			} catch (error) {
				if (!cleanupRef.current) {
		...
		<div>
			...
			{!isCompatibleWithTopicSource && (
				<div>
					...
					<p>
						Topic must be from ROS2 datasource and have a Webrtc
						server running
					</p>
				</div>
			)}
