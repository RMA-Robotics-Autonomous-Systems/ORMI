...
import { DatasourceTopic, SelectedTopic } from "@workspace/ormi-core/datasources";
 ...
 */
function createSelectedTopic(topic: DatasourceTopic, property: string = ""): SelectedTopic {
	return {
		topic: topic.topic,
 ...
 */
const ImagePreview = memo(function ImagePreview({ topic }: { topic: DatasourceTopic }) {
	const imageWidget = useMemo(() => ImageViewerDefinition(), []);

	// Extract stable values from topic for dependency array
	const { topic: topicName, datasource_id, type, rawType, bufferSize, source } = topic;

	// Memoize the props to prevent re-subscription on every render
	const widgetProps = useMemo(() => ({
		title: topicName,
		topic: createSelectedTopic({ topic: topicName, datasource_id, source, type, rawType, bufferSize }),
 ...
 * Props are memoized to prevent re-subscription in LocalDataSourcesProvider
 */
const JsonPreview = memo(function JsonPreview({ topic }: { topic: DatasourceTopic }) {
 ...
 * Props are memoized to prevent re-subscription in LocalDataSourcesProvider
 */
const ConditionalPreview = memo(function ConditionalPreview({ topic }: { topic: DatasourceTopic }) {
	const conditionalWidget = useMemo(() => CondStatusIndicatorDefinition(), []);
	...
	const widgetProps = useMemo(() => ({
		...
		topic: createSelectedTopic({ topic: topicName, datasource_id, source, type, rawType, bufferSize }),
		status: [
			{ name: "False", color: "#ef4444", condition: "<" as const, value: 0.5 },
			{ name: "True", color: "#22c55e", condition: ">=" as const, value: 0.5 },
		],
	}), [topicName, datasource_id, source, type, rawType, bufferSize]);
 ...
 * Props are memoized to prevent re-subscription in LocalDataSourcesProvider
 */
const EchartPreview = memo(function EchartPreview({ topic }: { topic: DatasourceTopic }) {
 ...
 * Props are memoized to prevent re-subscription in LocalDataSourcesProvider
 */
const IMUPreview = memo(function IMUPreview({ topic }: { topic: DatasourceTopic }) {
	const echartWidget = useMemo(() => ChartEchartsWidgetDefinition(), []);
		...
		series: [
			{
				title: "X",
				fill: false,
				topic: createSelectedTopic({ topic: topicName, datasource_id, source, type, rawType, bufferSize }, "angular_velocity.x"),
				name: "X",
				color: "#ef4444",
				smooth: true,
			},
			{
				title: "Y",
				fill: false,
				topic: createSelectedTopic({ topic: topicName, datasource_id, source, type, rawType, bufferSize }, "angular_velocity.y"),
				name: "Y",
				color: "#22c55e",
				smooth: true,
			},
			{
				title: "Z",
				fill: false,
				topic: createSelectedTopic({ topic: topicName, datasource_id, source, type, rawType, bufferSize }, "angular_velocity.z"),
				name: "Z",
				color: "#3b82f6",
				smooth: true,
			},
 ...
 * Props are memoized to prevent re-subscription in LocalDataSourcesProvider
 */
const PathPreview = memo(function PathPreview({ topic }: { topic: DatasourceTopic }) {
 ...
 * Props are memoized to prevent re-subscription in LocalDataSourcesProvider
 */
const PointCloudPreview = memo(function PointCloudPreview({ topic }: { topic: DatasourceTopic }) {
...
export function registerDefaultTopicPreviews() {
	...
	topicPreviewRegistry.register("boolean", {
		component: (topic: DatasourceTopic) => <ConditionalPreview topic={topic} />,
		minHeight: "80px",
	});
	...
	topicPreviewRegistry.register("PointsCloud", {
		component: (topic: DatasourceTopic) => <PointCloudPreview topic={topic} />,
		minHeight: "250px",
	});
