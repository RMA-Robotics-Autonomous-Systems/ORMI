...
export function ChartEchartsWidget(props: ChartEchartsSettings) {
	...
	function buildOption(): echarts.EChartsOption {
		...
		props.series.forEach((s) => {
			...
			const fallbackName = s.title?.trim()
				? s.title
				: s.topic.property
					? `${s.topic.topic} (${s.topic.property})`
					: s.topic.topic;
			if (!axisConfigs[key]) {
				axisConfigs[key] = {
					type: "value",
		...
		props.series.forEach((s) => {
			...
			let data = topicData.map((d) => ({
				value: [d.time * 1000, d.value],
			}));
			// Always add a null point at current time to keep time axis advancing
			if (
				data.length === 0 ||
				(data[data.length - 1]?.value?.[0] ?? 0) < now
			) {
		...
		options: {
			detail: {
				elements: [
					...
					{
						type: "Control",
						scope: "#/properties/axis/properties/position",
					},
				],
			},
		},
		...
		options: {
			detail: {
				elements: [
					...
					{
						type: "Control",
						scope: "#/properties/lineStyle/properties/type",
					},
				],
			},
		},
