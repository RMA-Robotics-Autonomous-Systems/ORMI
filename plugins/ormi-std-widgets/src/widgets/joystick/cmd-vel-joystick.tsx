...
export function JoypadControls(props: JoypadControlsProps) {
		...
		if (!publisher) {
			const timer = setTimeout(() => {
				if (!publishers.get(selectedTopic.topic)) {
					toast.error(
						`JoypadControls: Topic "${selectedTopic.topic}" not found in publishers.`,
					);
				}
			}, 1000);
			...
			if (Array.isArray(props.axes)) {
				for (const axisConfig of props.axes) {
					...
					if (value !== 0) {
						...
						if (type === "linear") {
							if (axis === "x")
								movement.linear.x = value * speed * multiplier;
							else if (axis === "y")
								movement.linear.y = value * speed * multiplier;
							else if (axis === "z")
								movement.linear.z = value * speed * multiplier;
						} else if (type === "angular") {
							if (axis === "x")
								movement.angular.x = value * speed * multiplier;
							else if (axis === "y")
								movement.angular.y = value * speed * multiplier;
							else if (axis === "z")
								movement.angular.z = value * speed * multiplier;
						}
					}
				}
			}
	...
	const handleAxisChange = useCallback(
		(axisConfig: { axis: string }, value: number, isPositive: boolean) => {
