...
export function KeyBoardControl(props: KeyboardControlData) {
		...
		if (!publisher) {
			const timer = setTimeout(() => {
				if (!publishers.get(selectedTopic.topic)) {
					toast.error(
						`KeyboardControl: Publisher for topic ${selectedTopic.topic} not found. Please check your configuration.`,
					);
				}
			}, 1000);
			...
			if (Array.isArray(props.axes)) {
				for (const axisConfig of props.axes) {
					...
					const positiveActive =
						axisKeyRefs.current[positiveKey] || false;
					const negativeActive =
						axisKeyRefs.current[negativeKey] || false;
					...
					if (axisValue !== 0) {
						...
						if (type === "linear") {
							if (axis === "x") movement.linear.x += finalValue;
								...
								movement.linear.y += finalValue;
							else if (axis === "z")
								movement.linear.z += finalValue;
						} else if (type === "angular") {
							if (axis === "x") movement.angular.x += finalValue;
							else if (axis === "y")
								movement.angular.y += finalValue;
							else if (axis === "z")
								movement.angular.z += finalValue;
						}
					}
				}
			}
	...
	const axisKeyHandlers = useMemo(() => {
		const handlers: Record<
			string,
			{ handleActive: () => void; handleInactive: () => void }
		> = {};
	...
	const handleIncSpeedActive = useCallback(
		() => setSpeedKeyIncActive(true),
		[],
	);
	const handleIncSpeedInactive = useCallback(
		() => setSpeedKeyIncActive(false),
		[],
	);
	const handleDecSpeedActive = useCallback(
		() => setSpeedKeyDecActive(true),
		[],
	);
	const handleDecSpeedInactive = useCallback(
		() => setSpeedKeyDecActive(false),
		[],
	);
		...
		<div className="flex flex-col h-full overflow-auto">
			<div className="flex flex-col justify-start items-center p-1 sm:p-2 lg:p-4 h-full gap-1 sm:gap-2 lg:gap-3 min-h-0">
				...
				<div className="grid grid-cols-1 gap-2 sm:gap-3 lg:gap-4 w-full flex-1 min-h-0 overflow-auto">
					{Array.isArray(props.axes) ? (
						props.axes.map((axisConfig, index) => {
							const positiveKey = `${axisConfig.axis}_positive`;
							const negativeKey = `${axisConfig.axis}_negative`;
							const positiveActive =
								axisKeyStates[positiveKey] || false;
							const negativeActive =
								axisKeyStates[negativeKey] || false;
							...
							const positiveHandlers =
								axisKeyHandlers[positiveKey];
							const negativeHandlers =
								axisKeyHandlers[negativeKey];

							return (
								<div
									key={index}
									className="flex flex-col items-center p-1 sm:p-2 border rounded min-h-0"
								>
									<div className="text-xs sm:text-sm font-medium mb-1 sm:mb-2 truncate w-full text-center">
										{axisConfig.axis}
									</div>
									<div className="flex justify-around items-center w-full gap-2 sm:gap-4">
										<div className="flex flex-col items-center min-w-0">
											...
											<DigitalComponent
												digitalInput={
													axisConfig.key_positive
												}
												onActive={
													positiveHandlers
														? positiveHandlers.handleActive
														: () => {}
												}
												onInactive={
													positiveHandlers
														? positiveHandlers.handleInactive
														: () => {}
												}
											/>
										</div>
										<div className="flex flex-col items-center min-w-0">
											...
											<DigitalComponent
												digitalInput={
													axisConfig.key_negative
												}
												onActive={
													negativeHandlers
														? negativeHandlers.handleActive
														: () => {}
												}
												onInactive={
													negativeHandlers
														? negativeHandlers.handleInactive
														: () => {}
												}
											/>
										</div>
