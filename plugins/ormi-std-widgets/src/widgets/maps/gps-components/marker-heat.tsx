...
export default function HeatMarker(props: {
	...
	useEffect(() => {
		...
		const num_data = props.numericalTopic
			? getSource(props.numericalTopic)
			: null;
		if (!loc_data) {
			return;
		}
				...
				if (bestMatch) {
					...
					associatedValue =
						typeof bestMatch === "number"
							? bestMatch
							: (bestMatch as any)?.value ||
								(bestMatch as any)?.data ||
								1;
				}
			}
	...
	const values = locations.map(
		(loc: { coords: [number, number]; value: number }) => loc.value,
	);
	...
	const pointFeatures: GeoJSON.FeatureCollection = {
		...
		features: sortedLocations.map(
			(loc: { coords: [number, number]; value: number }) => ({
				type: "Feature",
				geometry: {
					type: "Point",
				...
				>
					<div className="rounded-lg shadow-md p-3 bg-card text-card-foreground">
						...
						<div className="flex items-center space-x-2">
							...
							<span className="text-xs">
								{hoveredPoint.coords[0].toFixed(6)}
							</span>
						</div>
						<div className="flex items-center space-x-2">
							<span className="text-xs font-medium">
							...
							<span className="text-xs">
								{hoveredPoint.coords[1].toFixed(6)}
							</span>
						</div>
					</div>
				</Popup>
