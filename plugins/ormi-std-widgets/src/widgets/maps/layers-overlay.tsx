  ...
  const handleLayerCenter = async (layer: CustomLayer, index: number) => {
    ...
    try {
      ...
      if (bounds) {
        ...
        console.log(
          `Layer "${layer.name}" bounds not available. Consider adding bounds information.`,
        );
      }
    } catch (error) {
                ...
                <div className="flex flex-col gap-1 min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      className="p-1 h-6 w-6"
                      onClick={() =>
                        onLayerVisibilityChange(index, !layer.visible)
                      }
                    >
                      {layer.visible ? (
                  ...
                  {layer.visible && (
                    <div className="flex items-center gap-2 w-full">
                      ...
                      <Slider
                        value={[layer.opacity]}
                        onValueChange={(value) =>
                          onLayerOpacityChange(index, value[0] ?? 0)
                        }
                        max={1}
                        min={0}
                        step={0.1}
