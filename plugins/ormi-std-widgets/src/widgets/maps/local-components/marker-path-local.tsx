...
export default function PathLocalMarker(props: LocalTopicVisualizerProps) {
    ...
    const gpsOriginData: GeolocationPosition | null = (() => {
        ...
        try {
            const lastGpsData = gpsOriginSourceData.data[gpsOriginSourceData.data.length - 1] as GeolocationPosition;
            return lastGpsData;
        } catch (error) {
    ...
    useEffect(() => {
        ...
        if (!hasTransform) {
            if (transformError) {
                console.error(`Transform error for path ${name}: ${transformError}`);
        ...
        try {
            // Get the latest path
            const latestPath = sourceData.data[sourceData.data.length - 1] as Path;

            if (!latestPath || !latestPath.poses || latestPath.poses.length === 0) {
                return;
            }
            ...
            for (const pose of latestPath.poses) {
                ...
                if (gpsCoord) {
                    ...
                    transformedCoords.push([gpsCoord.longitude, gpsCoord.latitude]);
                }
            }
    ...
    }, [sourceData, gpsOriginData, hasTransform, hasGPSOrigin, transformError, transformPointToGPS, name, uniqueTopicId, show]);
