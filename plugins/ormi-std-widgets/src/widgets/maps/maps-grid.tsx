...
export default function MapsGrid({ mapRef, showGrid, onGridUpdate }: MapsGridProps) {
    ...
    const createGridLines = (bounds: [number, number, number, number], gridSizeMeters: number = 1) => {
    ...
    const updateGridForViewport = () => {
        ...
        if (zoom >= 20) gridSizeMeters = 1;       // 1m
        else if (zoom >= 18) gridSizeMeters = 10;  // 10m
        else if (zoom >= 16) gridSizeMeters = 100; // 100m
        else if (zoom >= 14) gridSizeMeters = 1000; // 1km
        else if (zoom >= 12) gridSizeMeters = 10000; // 10km
...
export const useMapGrid = (mapRef: React.RefObject<MapRef | null>, showGrid: boolean) => {
...
export const GridUtils = {
    ...
    metersToDegreesLng: (meters: number, latitude: number) => meters / (111320 * Math.cos(latitude * Math.PI / 180)),
    createGridLines: (bounds: [number, number, number, number], gridSizeMeters: number = 1) => {
        const [minLng, minLat, maxLng, maxLat] = bounds;
        const features = [];
    ...
    getGridSizeForZoom: (zoom: number) => {
        if (zoom >= 20) return 1;       // 1m
        else if (zoom >= 18) return 10;  // 10m
        else if (zoom >= 16) return 100; // 100m
        else if (zoom >= 14) return 1000; // 1km
        else if (zoom >= 12) return 10000; // 10km
