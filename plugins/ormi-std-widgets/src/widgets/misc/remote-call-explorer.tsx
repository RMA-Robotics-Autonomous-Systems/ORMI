...
function StatusBadge({ status }: { status: RemoteCallStatus }) {
    const config: Record<RemoteCallStatus, { variant: "default" | "secondary" | "destructive" | "outline"; icon: React.ReactNode; label: string }> = {
        pending: { variant: "secondary", icon: <Clock className="w-3 h-3" />, label: "Pending" },
        executing: { variant: "default", icon: <Loader2 className="w-3 h-3 animate-spin" />, label: "Executing" },
        succeeded: { variant: "default", icon: <CheckCircle2 className="w-3 h-3" />, label: "Succeeded" },
        failed: { variant: "destructive", icon: <XCircle className="w-3 h-3" />, label: "Failed" },
        canceled: { variant: "outline", icon: <Square className="w-3 h-3" />, label: "Canceled" },
    };
...
function FeedbackDisplay({ feedback, feedbackType, feedbackSchema }: FeedbackDisplayProps) {
    ...
    const displayType = useMemo(() => {
        ...
        if (feedbackType) {
            ...
            if (lowerType.includes("number") || lowerType.includes("float") || lowerType.includes("int") || lowerType.includes("progress")) {
                return "progress";
            }
        ...
        <div className="space-y-2">
            ...
            {displayType === "progress" && (
                <div className="space-y-1">
                    <Progress value={Math.min(100, Math.max(0, Number(feedback)))} className="h-2" />
                    <p className="text-xs text-muted-foreground text-right">{Number(feedback).toFixed(1)}%</p>
                </div>
            )}
            {displayType === "text" && (
                <p className="text-sm bg-muted p-2 rounded-md">{String(feedback)}</p>
            )}
            {displayType === "json" && (
...
function ResultDisplay({ result, error, duration, status }: ResultDisplayProps) {
...
function CallExecutor({ definition }: CallExecutorProps) {
    ...
    const hasRequestFields = requestSchema.properties && Object.keys(requestSchema.properties).length > 0;
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    {definition.feedbackType && (
                        <Badge variant="outline" className="text-xs">Has Feedback</Badge>
                    )}
                    {definition.cancelable && (
                        <Badge variant="outline" className="text-xs">Cancelable</Badge>
                    )}
                </div>
            </div>
                    ...
                    {showRequestForm && (
                        <div className="border rounded-md p-3">
                            <JsonForms
                                ...
                                cells={shadcnCells}
                                onChange={({ data }) => setRequestData(data || {})}
                            />
                        </div>
                    )}
                </div>
        ...
        <div className="border rounded-lg overflow-hidden">
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm truncate" title={definition.name}>
                        {shortName}
                    </p>
                    <p className="text-xs text-muted-foreground truncate" title={definition.name}>
                        {definition.name}
                    </p>
                </div>
                ...
                <div className="p-3 space-y-3 bg-muted/10">
                    ...
                    <div className="text-xs space-y-1">
                        <div className="flex gap-2">
                            <span className="text-muted-foreground w-16">Request:</span>
                            <code className="bg-muted px-1 rounded truncate flex-1" title={definition.rawRequestType}>
                                {definition.rawRequestType || "unknown"}
                            </code>
                        </div>
                        <div className="flex gap-2">
                            <span className="text-muted-foreground w-16">Response:</span>
                            <code className="bg-muted px-1 rounded truncate flex-1" title={definition.rawResponseType}>
...
function RemoteCallExplorer({ }: RemoteCallWidgetProps) {
    ...
    const getCallId = (call: RemoteCallDefinition) => `${call.datasource_id}::${call.name}`;
        ...
        <div className="h-full flex flex-col overflow-hidden">
            ...
            <div className="flex-1 min-h-0 overflow-auto">
                <div className="p-3 space-y-4">
                    {Array.from(groupedCalls.entries()).map(([datasource, dsCalls]) => (
                            ...
                            <div className="space-y-2">
                                {dsCalls.map((call) => (
                                    <ServiceCard
                                        key={getCallId(call)}
                                        definition={call}
                                        isExpanded={expandedCallId === getCallId(call)}
                    ...
                    {isEmpty && (
                        <div className="text-center text-muted-foreground py-12">
                            ...
                            <p className="font-medium">No remote calls available</p>
                            <p className="text-xs mt-1">Connect a datasource with services</p>
                        </div>
                    )}
                </div>
            </div>
...
export function RemoteCallExplorerDefinition(): WidgetDefinition {
    return {
        ...
        description: "List and execute available remote calls (services/actions)",
        titleProp: "title",
        icon: <PhoneCall />,
