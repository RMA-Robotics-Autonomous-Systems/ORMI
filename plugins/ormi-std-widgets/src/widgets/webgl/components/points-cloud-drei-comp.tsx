...
const PointsSourceRenderer = ({
  ...
  const processData = useCallback(() => {
    ...
    if (
      ...
      transformChain =
        findTransformChain(transformsTrees, refFrame, settings.targetFrame) ??
        null;
    }
    ...
    if (settings.rollingBuffer) {
      ...
      for (let i = startIndex; i < dataArray.length; i++) {
        ...
        const messageTimeSeconds =
          (messageTime - startTimeRef.current) / 1000.0;
  ...
  const updateMaterial = useCallback(() => {
    ...
    if (mat.uniforms.useIntensity)
      mat.uniforms.useIntensity.value = settings.colorMode === "reflectivity";
