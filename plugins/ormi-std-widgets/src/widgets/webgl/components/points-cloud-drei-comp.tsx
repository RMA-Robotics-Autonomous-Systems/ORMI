...
import { PointsCloud } from '@workspace/ormi-core/types';
...
export const PointsCloudComp = (props: PointsCloudProps) => {
        ...
        if (last_value?.points?.length) {
            if (rollingBuffer) {
                ...
                const newPoints = last_value.points.map((p, idx) => ({
                    ...
                    color: last_value.colors?.[idx] ?
                        new THREE.Color(last_value.colors[idx].r, last_value.colors[idx].g, last_value.colors[idx].b) :
                        undefined,
    ...
    const pointsColors = pointsBuffer.length > 0 && pointsBuffer.some(p => p.color)
        ? pointsBuffer.map(p => p.color).filter(Boolean) as THREE.Color[]
        : undefined;
