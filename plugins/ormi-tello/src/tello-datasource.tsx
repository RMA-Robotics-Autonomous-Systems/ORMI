...
const TelloSourceProvider = (props: TelloSourceSettings) => {
	...
	const advertiserRef = useRef<Map<string, TelloAdvertiser>>(
		new Map<string, TelloAdvertiser>(),
	);
		...
		const waitTimeOut = setTimeout(() => {
						...
						Object.keys(telloTopics).forEach((topicKey) => {
							...
							if (subscribersCountRef.current.has(topicKey)) {
								...
								switch (topicKey) {
									case "imu":
										pluginsManager.doAction(
											`${datasource_id}-${topicKey}-published`,
											telloState.imu,
											Date.now(),
										);
										break;
			...
			pluginsManager.addAction(subscribe_hook, {
				...
				action: (topic: SelectedTopic) => {
					...
					if (!telloTopics[topic.topic]) {
						console.warn(
							`Topic ${topic.topic} does not exist in Tello datasource`,
						);
						return;
					}
			...
			pluginsManager.addAction(unsubscribe_hook, {
				...
				action: (topic: SelectedTopic, ignoreCount = false) => {
					...
					if (
						!ignoreCount &&
						subscribersCountRef.current.has(topic.topic)
					) {
						const count =
							subscribersCountRef.current.get(topic.topic) || 0;
						if (count > 1) {
			...
			pluginsManager.addFilter(advertise_hook, {
				...
				filter: async (topic: SelectedTopic): Promise<boolean> => {
					...
					if (topic.topic !== "movement") {
						console.warn(
							`Topic ${topic.topic} cannot be advertised`,
						);
						return false;
					}
					...
					if (
						advertiserRef.current &&
						advertiserRef.current.has(topic.topic)
					) {
						const advertiser = advertiserRef.current.get(
							topic.topic,
						);
						if (advertiser) {
							advertiser.count = (advertiser.count || 0) + 1;
							console.warn(
								`Advertiser ${topic.topic} already exist, count: ${advertiser.count}`,
							);
						}
						return true;
					}
							...
							action: (
								...
								if (
                                    ...
                                    */
									...
									const rc =
										"rc " +
										message.linear.y * 100 +
										" " +
										message.linear.x * 100 +
										" " +
										message.linear.z * 100 +
										" " +
			...
			pluginsManager.addAction(unadvertise_hook, {
				...
				action: async (
					...
					if (topic.topic !== "movement") {
						console.warn(
							`Topic ${topic.topic} cannot be unadvertised`,
						);
						return;
					}
		...
		return () => {
			...
			if (advertiserRef.current) {
				advertiserRef.current.forEach((advertiser) => {
					pluginsManager.removeAction(
						`${datasource_id}-${advertiser.topic}-publish`,
					);
				});
			}
