...
export function TelloCommandsControl(props: TelloCommandsControlData) {
  ...
  const keyStyleNames =
    "bg-black/10 p-[5%] w-full rounded-[var(--radius)] border-[0.2rem] border-black/10 flex justify-center items-center select-none hover:bg-black/20 hover:scale-110 hover:cursor-pointer data-[active=true]:bg-green-600/20 data-[active=true]:scale-110 transition-all duration-100";

  return (
    <div
      style={{ display: "grid", gap: "1rem", width: "100%", height: "100%" }}
    >
        ...
        } as ControlElement,
        {
          ...
          options: {
            ...
            asyncFunction: async () => {
              ...
              ).filter((ds) => ds.datasource_id === "tello-data-source");
