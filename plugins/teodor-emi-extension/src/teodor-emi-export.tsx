import { DatasourceTopicFilter, SelectedTopic } from "@workspace/ormi-core/datasources";
...
export const addTopicTypeFilter = (topicFilter: DatasourceTopicFilter) => {
    ...
    if (!topicFilter.rawType) {
        ...
        topicFilter.rawType = new RegExp(`(${topicFilter.rawType.source}|emi_msgs/msg/EMIGnss)`);
    }
...
export const mapMarkerComponent = (current_component: JSX.Element, t: MapTopic) => {
    if (t.makerType !== "TeodorEMI") {
        return current_component;
    }

    return <TeodorEmiMap key={t.name} topic={t.topic} name={t.name} scale={1} useRaw2={false} />;
}