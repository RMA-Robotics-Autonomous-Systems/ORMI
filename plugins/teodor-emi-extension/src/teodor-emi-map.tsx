...
export default function TeodorEmiMap(props: {
	...
	useEffect(() => {
		...
		if (emi_data.data.length > 0) {
			...
			if (
				emiMessage &&
				emiMessage.emi_array &&
				messageTime !== undefined
			) {
				const newPoints = emiMessage.emi_array.map(
					(sensor: EmiSensor) => ({
						coords: [
							sensor.gnss.latitude,
							sensor.gnss.longitude,
						] as [number, number],
				...
				setLocations((prevLocations) => {
					...
					newPoints.forEach((newPoint) => {
						...
						const sensorHistory = updatedLocations.filter(
							(loc) => loc.sensorId === newPoint.sensorId,
						);
						...
						if (sensorHistory.length > 0) {
							...
							if (lastPoint) {
								...
								if (
									dist > 0.1 ||
									newPoint.alert !== lastPoint.alert
								) {
